...
function getlog()
{
    ...
    if($action){
        writelog(SABLOG_ROOT . 'cache/log/adminlog.php', "<?PHP exit('Access Denied'); ?>\t$timestamp\t$sax_user\t$onlineip\t" .
                                                         htmlspecialchars(trim($action)) . "\twap\n");
    }
}
...
function wap_output()
{
    ...
    $content = preg_replace_callback("/\<a(\s*[^\>]+\s*)href\=([\"|\']?)([^\"\'\s]+)/ies", function(&$t) {
        return transhash($t[3], "<a{$t[1]}href={$t[2]}");
    }, $content);
    ob_end_clean();
...
function multi($num, $perpage, $curpage, $mpurl)
{
    ...
    if($num > $perpage){
        ...
        $multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="' . $mpurl . 'page=1">第一页</a> ' : '') .
        ...
        $multipage .= ($curpage < $pages ? '<a href="' . $mpurl . 'page=' . ($curpage + 1) . '">下一页</a>' : '') .
                      ($to < $pages ? ' <a href="' . $mpurl . 'page=' . $pages . '">最后一页</a>' : '');
