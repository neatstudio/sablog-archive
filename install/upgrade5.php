...
if ($action == 'first' || $action == 'second' || $action == 'three') {
...
<?php
if ($action == 'first') {
	$jumpurl = $php_self.'?action=second';
	...
	echo '<p class="p2">添加数据表完毕.</p><p class="p2"><a href="'.$jumpurl.'">程序将自动跳转.如果没有自动跳转,请点击这里.</a></p>';
	echo '</div></body></html>';
	exit;
} elseif ($action == 'second') {
	$jumpurl = $php_self.'?action=three';
	...
	echo '<meta HTTP-EQUIV="REFRESH" content="3;URL='.$jumpurl.'">';
	echo '<div class="install_main">';
	echo '<p class="p2">转换数据完毕.</p><p class="p2"><a href="'.$jumpurl.'">程序将自动跳转.如果没有自动跳转,请点击这里.</a></p>';
	echo '</div></body></html>';
	exit;
} elseif ($action == 'three') {
...
<div class="install_main">
  ...
  <div id="install_innertext">
    ...
    <p class="p2">当前版本为 <u>SaBlog-X v 1.5</u></p>
    <p class="p2">目标版本为 <u>SaBlog-X v 1.6</u></p>
    <p class="p2">升级过程完全不用人工干预,请耐心等待成功带来的喜悦.</p>
	<p class="p2"><a href="<?=$php_self?>?action=first">开始升级</a></p>
  </div>
</div>
<?php
}
