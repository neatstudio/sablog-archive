...
//文章列表
if(in_array($action, ['index', 'finduser', 'search', 'tags'])){
    session_start();
    // 检查浏览模式
    ...
    if($action == 'finduser'){
        ...
        if(!$searchid){
            ...
            $tatol = $search['tatols'];
            $query_sql .= " AND a.articleid IN (" . $search['ids'] . ") ORDER BY a." . $search['sortby'] . " " . $search['orderby'] . " LIMIT $start_limit, " .
                          $pagenum;
    ...
    if($tatol){
        ...
        while($article = $DB->fetch_array($query)){
            ...
            if($viewmode == 'normal'){
                if($article['readpassword'] && ($_SESSION['readpassword_' . $article['articleid']] != $article['readpassword']) && $sax_group != 1 &&
                   $sax_group != 2){
...
elseif($_GET['action'] == 'show'){
    ...
    $options['meta_keywords'] = $article['cname'] . ',' . $article['username'] . ',' . ($article['keywords'] ? $article['keywords'] . ',' : '') .
                                $article['title'] . ',' . $options['meta_keywords'];
    $options['meta_description'] = $article['cname'] . ',' . $article['username'] . ',' . ($article['keywords'] ? $article['keywords'] . ',' : '') .
                                   $article['title'] . ',' . $options['meta_description'];
    $options['title_keywords'] = ' - ' . $article['cname'] . ',' . ($article['keywords'] ? $article['keywords'] . ',' : '') . $article['username'] . ',' .
                                 $options['title_keywords'];
    ...
    if($article['readpassword'] && ($_SESSION['readpassword_' . $articleid] != $article['readpassword']) && $sax_group != 1 && $sax_group != 2){
                ...
                foreach($attachs AS $attach){
                    ...
                    if(file_exists($a_path)){
                        ...
                        if($a_ext == 'gif' || $a_ext == 'jpg' || $a_ext == 'jpeg' || $a_ext == 'png'){
                            ...
                            // 如果非图片文件
                            $a_size = sizecount($attach['filesize']);
                            $article['file'][$attach['attachmentid']] = [$attach['attachmentid'], $attach['filename'], $a_size, $attach['downloads']];
                        }
                    }
                }
                //如果空,释放掉变量
        ...
        if($article['keywords']){
            ...
            if($options['related_shownum']){
                ...
                if($related_tatol > 1 && $relids != $articleid){
                    $order = in_array($options['related_order'], ['dateline', 'views', 'comments']) ? $options['related_order'] : 'dateline';
                    $query = $DB->query("SELECT articleid,title,views,comments FROM {$db_prefix}articles WHERE visible='1' AND articleid IN ($relids) ORDER BY " .
                                        $order . " DESC LIMIT " . intval($options['related_shownum']));
                    $titledb = [];
        ...
        if($options['enable_trackback'] && $article['trackbacks']){
            ...
            $query = $DB->query("SELECT title,dateline,excerpt,url,blog_name FROM {$db_prefix}trackbacks WHERE visible='1' AND articleid='$articleid' ORDER BY trackbackid " .
                                $tborder);
            ...
            while($comment = $DB->fetch_array($query)){
                ...
                if($comment['url']){
                    if(isemail($comment['url'])){
                        ...
                        $comment['author'] = "<a href=\"javascript:navigate('mai' + 'lto:' + '" . $front . "' + '@' + '" . $back . "')\" target=\"_blank\">" .
                                             $comment['author'] . "</a>";
                    } else{
