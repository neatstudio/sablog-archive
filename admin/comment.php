...
if($action == 'domorecmlist') {
	...
	if ($cids = implode_ids($_POST['comment'])) {
                $articles = [];
		if ($del) {
                        $query = $DB->query("SELECT a.articleid FROM {$db_prefix}comments c LEFT JOIN {$db_prefix}articles a ON (a.articleid=c.articleid) and c.commentid IN($cids)");
		...
		if($aids){
		$aids = implode_ids($aids);
		$query = $DB->query("SELECT articleid FROM {$db_prefix}articles WHERE articleid IN ($aids)");
		while ($article = $DB->fetch_array($query)) {
			// 更新所有文章的评论数
